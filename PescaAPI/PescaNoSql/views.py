...
def metodoApi(request, id=0):
    if request.method == 'GET':
        ...
        metodo_data = JSONParser().parse(request)
        metodo = Metodo.objects.get(ID_METODO=metodo_data["ID_METODO"])
        ...
        print("codigo",id)
        metodo = Metodo.objects.get(ID_METODO=id)
        metodo.delete()
...
def cuencasApi(request, id=0):
    if request.method == 'GET':
        ...
        cuenca_data = JSONParser().parse(request)
        cuenca = Cuenca.objects.get(ID_CUENCA=id)
...
def pescasApi(request, id=0):
    if request.method == 'GET':
        ...
        pescas_data = JSONParser().parse(request)
        pescas = Pesca.objects.get(ID_PESCA=id)
